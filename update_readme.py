...
def table_datasets():
    ...
    dataset_table = ["Dataset", "Data Source", "Date Range", "Download", "Preview"]
    columns = len(dataset_table)
    
    link = mdFile.new_inline_link
    for ds in datasets:
        dataset_table.extend([link(link=ds['dataset_detail'], text=ds['dataset']), ds['data_source'], ds['date_range'], link(link=ds['link_csv'], text='csv', align='center'), link(link=ds['link_preview'], text='preview', align='center')])
